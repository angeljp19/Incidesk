...
import { TicketFeedbackModal } from "../../components/ticketFeedBack";
...
export function SolicitudTicketPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (t.solicitante_id !== userObj.id) {
        navigate("/user/solicitudes");
      }
      setTicket(t);

      const coms = await ComentariosAPI.getComentariosById(id);
      setComentarios(coms.filter((c: any) => !c.es_nota_interna));
       if (t && t.EstadoTicket.id === 4 && !t.calificacion
  ...
  const handleFeedbackSubmit = async (calificacion: "Muy buena" | "Buena" | "Regular" | "Mala") => {
    await TicketAPI.calificar(ticket!.id, calificacion);
    setTicketFeedBack(false);
  }

  if (!ticket) return <IncideskSkeleton />;

  console.log(ticketFeedBack);
  console.log(ticket);

  return (
    <div className="flex flex-col md:flex-row gap-6 lg:p-5 w-full h-full overflow-auto">
      {/* PANEL IZQUIERDO — INFORMACIÓN DEL TICKET */}
      <div className="bg-white shadow-lg rounded-2xl p-6 space-y-6 lg:overflow-auto no-scrollbar w-full lg:w-1/3">
        {/* HEADER */}
        ...
        <div className="bg-[#F5F7FB] rounded-xl p-4 text-sm space-y-2">
          <p>
            ...
            <span className="text-gray-700">
              {formatFechaVE(ticket.fecha_creacion)}
            </span>
          </p>
          ...
          <p>
            ...
            <span className="text-gray-700">
              {ticket.fecha_ultima_actualizacion
                ? formatFechaVE(ticket.fecha_ultima_actualizacion)
                : "Sin actualización"}
            </span>
          </p>
          ...
          {ticket.fecha_cierre && (
            <p>
              ...
              <span className="text-gray-700">
                {formatFechaVE(ticket.fecha_cierre)}
              </span>
            </p>
          )}
        </div>
      ...
      <div className="bg-white shadow-lg rounded-2xl p-6 flex flex-col w-full h-full">
        ...
        <div className="flex bg-[#F5F7FB] rounded-xl p-1 mb-4">
          <button className="flex-1 py-2 text-sm rounded-lg transitionbg-white shadow text-blue-600 font-semibold">
      ...
      {ticketFeedBack && (
        <TicketFeedbackModal
          open={ticketFeedBack}
          onClose={() => setTicketFeedBack(false)}
